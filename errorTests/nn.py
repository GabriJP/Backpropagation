...
def net_1capa(learning_rate, number_of_hidden_elements, noise, func, momentum=0.9, smartgit=2):
    if func==2:
    ...
    csvfile = open('datos r:'+ str(learning_rate) + ' n:' + str(number_of_hidden_elements) + ' mom:' + str(momentum) +'.csv', 'w')
    ...
    while errors[-1] > TARGET_ERROR and len(errors) < MAX_ITERATIONS:
        ...
        writer.writerow([errors[-1]])

    errorWithNoise = sess.run(cross_entropy, feed_dict={x: add_noise(x_data, 0.1), y_: y_data})
    if len(errors) >= MAX_ITERATIONS:
        return errorWithNoise
    else:
        return errorWithNoise
